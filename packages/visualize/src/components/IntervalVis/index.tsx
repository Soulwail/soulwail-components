...
const IntervalVis: React.FC = () => {
  const { categoryList = [], colorCategoryList = [] } =
  ...
  return (
    <>
      ...
      <Form.Item
        label="横轴（聚合字段）"
        name={['encode', 'x']}
        rules={[{ required: true, message: '请选择横轴字段' }]}
      >
      ...
      <Form.Item
        label={<Text type="secondary">排序依据</Text>}
      ...
      <Form.Item
        label={<Text type="secondary">排序规则</Text>}
        ...
        {({ getFieldValue }) => {
          ...
          return encodeColor ? (
            <>
              ...
              >
                <Select options={colorCategoryList} placeholder="请选择" />
              </Form.Item>
